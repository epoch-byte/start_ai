...
### [最小二乘法]()
 为了方便叙述，我们将$J(w)$用矩阵形式表达，即
   $J(w) = \frac{1}{2}(\bf{X}w-Y)^{2} = \frac{1}{2}(\bf{X}w-Y)^{T}(\bf{X}w-Y)     \qquad (1.5)$
   其中，X为𝑚行𝑛+1列的矩阵(第一列全为1，即式(1.2)中的$x_{0}$​)，w为 𝑛+1行1列的矩阵(包含了$𝑤_{0}$​)，𝑌为𝑚行1列的矩阵。

 计算矩阵导数
$$
   \begin{align}
         \frac{\partial J(w)}{\partial w} &= \frac{1}{2}*\frac{\partial(\bf{X}w-Y)^{T}(\bf{X}w-Y)}{\partial w} \\
                                          &= \frac{1}{2}*\frac{\partial (w^{T}X^{T}Xw-Y^{T}Xw-w^{T}X^{T}Y+Y^{T}Y)}{\partial w} \\
                                          &= \frac{1}{2}*\frac{\partial (w^{T}X^{T}Xw)}{\partial w}-\frac{1}{2}*\frac{\partial (Y^{T}Xw)}{\partial w} -\frac{1}{2}*\frac{\partial (w^{T}X^{T}Y)}{\partial w}+\frac{1}{2}*\frac{\partial (Y^{T}Y)}{\partial w} \\
                                          &= \frac{1}{2}*\frac{\partial ((Xw)^{T}Xw)}{\partial w}-\frac{1}{2}*\frac{\partial ((X^{T}Y)^{T}w)}{\partial w} -\frac{1}{2}*\frac{\partial ((Y^{T}X)w)}{\partial w} + 0 \\
                                          &= \frac{1}{2}*(2*Xw*X))-\frac{1}{2}*X^{T}Y -\frac{1}{2}*(Y^{T}X)^{T} + 0 \\
                                          &= X^{T}Xw-X^{T}Y
   \end{align}
$$
令$\frac{\partial J(w)}{\partial w} = 0$, 则 $$w =(X^{T}X)^{-1}X^{T}Y$$
由上式可知，最小二乘法需要计算$(𝑋^{𝑇}𝑋)^{−1}$，但是矩阵求逆的时间复杂度为 $𝑂(𝑛^{3})$，因此当特征数量𝑛较大时，其运算代价非常大。所以这种方法只适用于特征数量较少的线性模型，不适用于其他模型。现代机器学习中常用的参数更新方法是$\color{Orange}{梯度下降法}$。
